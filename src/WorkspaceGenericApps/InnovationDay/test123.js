(function(){
	...
	Ext.define('Intel.InnovationDay', {
			...
			_saveVote:function(record){
				var votedStr;
				return _loadCurrentReleasesbyObjectId()
				.then(function(record){
					var recordChangedId = record.data.id;
					var result = JSON.parse(atob(me.currentRelease.data.Theme));
					_.each(result,function(r,key){
						if(r.data.id === recordChangedId)
						{
							r.data.voteCount.push(me.ideaOwnerUserObjectId);
							var votedStr = btoa(JSON.stringify(result, null, '\t'));
							me._setReleaseThemeRecord(votedStr);
							return;
						}
					});
